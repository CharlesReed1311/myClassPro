...
export default forwardRef<HTMLDivElement, DayCellProps>(function DayCell(
  ...
  // Debug: Log the day object to inspect scraped data
  console.log("DayCell Data:", day);

  // Enhanced isErrorDay: Treat "-" or holiday in event as error, but allow numeric dayOrder as working
  const isErrorDay = day?.dayOrder === "-" || (day.event?.includes("Holiday") ?? false);
  const isWorkingDay = !isErrorDay && (day.dayOrder === "" || /^\d+$/.test(day.dayOrder));
  ...
  return (
    <div
      ref={ref}
      aria-label={day?.date}
      title={`${day.date} - Day Order: ${day?.dayOrder ?? "N/A"}`}
    ...
    >
      ...
      <DayOrderDisplay dayOrder={day?.dayOrder} isToday={isToday} isWorkingDay={isWorkingDay} />
    </div>
  );
});
...
const HolidayDisplay: React.FC<HolidayDisplayProps> = ({
  ...
  return (
    <p
      className={`text-left break-words -mx-2 text-sm ${
        isErrorDay
          ? "text-light-error-color dark:text-dark-error-color"
          : "rounded-md border-l-2 border-r-0 border-light-info-color bg-light-info-background px-1 py-0.5 pl-2 text-light-info-color dark:border-dark-info-color dark:bg-dark-info-background dark:text-dark-info-color dark:opacity-70"
      }`}
    >
      {holiday.replaceAll(",", ", ")}
    </p>
...
const DayOrderDisplay: React.FC<DayOrderDisplayProps> = ({
  ...
  if (!isWorkingDay || dayOrder === "-") return null;
  return (
