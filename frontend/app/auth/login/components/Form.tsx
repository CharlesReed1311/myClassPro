...
export default function Form() {
	...
	const handleLogin = useCallback(async (account: string, password: string) => {
		// Clean up the account by removing @srmist.edu.in if present
		const cleanedAccount = account.replace("@srmist.edu.in", "").trim();
		...
		const login = await fetch(`${rotateUrl()}/login`, {
			...
			body: JSON.stringify({
				account: cleanedAccount, // Use the cleaned account
				password: password,
			}),
		});
		...
		>
			...
			<div className="flex flex-row gap-2">
				<Button
					...
					className={`w-full md:w-fit ${status === 2 ? "border border-light-success-color bg-light-success-background text-light-success-color dark:border-dark-success-color dark:bg-dark-success-background dark:text-dark-success-color" : status === 1 ? "border border-light-warn-color bg-light-warn-background text-light-warn-color dark:border-dark-warn-color dark:bg-dark-warn-background dark:text-dark-warn-color" : status === -1 ? "border border-light-error-color bg-light-error-background text-light-error-color dark:border-dark-error-color dark:bg-dark-error-background dark:text-dark-error-color" : ""}`}
					type="submit"
